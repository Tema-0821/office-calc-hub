  ...
  return (
    <CalculatorLayout
      ...
      guide={
        <>
          <p>
            엑셀로 가계부를 만들었다가 며칠 만에 손을 놓은 경험이 있다면, 여기서는 딱 두 가지만
            등록하면 됩니다. <strong>기본 설정</strong>에서 월급(실수령액 기준)과 매달 반복되는
            고정지출(월세, 구독료 등)을 등록하세요. <strong>지출 달력</strong>에서 날짜를 클릭하면
            그날 쓴 돈을 카테고리와 함께 기록할 수 있습니다.
          </p>
          <p>
          ...
          <p>
            <strong>사용 예시</strong>: 월급 300만원, 월세·구독료 등 고정지출 50만원을 등록하고
            이번 달 달력에 식비·교통비 등으로 20만원을 기록했다면, 이번 달에 남는 돈은 300만원 −
            50만원 − 20만원 = 230만원으로 자동 계산됩니다. 다음 달, 그다음 달도 같은 방식으로
            쌓여서 누적 잔고 추이 표에 이어집니다.
          </p>
          <p>
            <strong>할부 지출 활용 팁</strong>: 노트북이나 가전제품을 6개월 무이자 할부로
            구매했다면, 총 구매액과 개월수만 입력해두세요. 결제일 기준으로 매달 균등하게 지출에
            반영되다가, 할부가 끝나는 달 다음부터는 자동으로 지출 목록에서 빠지기 때문에 매달
            직접 지우거나 다시 입력할 필요가 없습니다.
          </p>
          <p>
            <strong>계산기 연동으로 놓치는 수입·지출 줄이기</strong>: 실업급여나 퇴직금처럼 특정
            달에 한 번 들어오는 큰 금액, 4대보험료처럼 매달 빠져나가는 고정 공제액을 잔고
            계산에서 깜빡하기 쉽습니다. 화면 오른쪽 계산기 연동 패널에서 해당 계산기를 먼저
            사용한 뒤 체크박스를 켜면, 수동으로 매번 입력하지 않아도 자동으로 반영됩니다.
          </p>
        </>
      }
      faq={[
