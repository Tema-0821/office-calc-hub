  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
      ...
      <div className="mt-4 flex flex-col gap-2">
        <ResultRow label="수입" value={formatWon(summary.income)} />
        <ResultRow label="고정지출" value={formatWon(summary.fixedTotal)} />
        <ResultRow label="변동지출 (달력 기록)" value={formatWon(summary.variableTotal)} />
        <ResultRow label="이번 달 순증감" value={formatWon(summary.netChange)} emphasis />
      </div>
      ...
      {budget > 0 && (
        <div className="mt-4">
          <div className="flex items-center justify-between text-xs text-zinc-500 dark:text-zinc-400">
            <span>예산 사용률</span>
            <span>
              {formatWon(summary.variableTotal)} / {formatWon(budget)}
            </span>
          </div>
          ...
          {overBudget && (
            <p className="mt-1 text-xs text-red-500">
              예산을 {formatWon(summary.variableTotal - budget)} 초과했습니다.
            </p>
          )}
        </div>
      )}
      ...
      <div className="mt-4">
        ...
        <div className="mt-2 flex flex-col gap-1.5">
          {EXPENSE_CATEGORIES.map((cat) => (
            <ResultRow key={cat.value} label={cat.label} value={formatWon(summary.categoryTotals[cat.value])} />
          ))}
        </div>
      </div>
      ...
      <div className="mt-4 border-t border-zinc-200 pt-4 text-xs text-zinc-500 dark:border-zinc-800 dark:text-zinc-400">
        <p>
          현재 지출 추세(하루 평균 {formatWon(projection.dailyAvgVariable)})가 이어진다면,{" "}
          {projection.nextYear}년 {projection.nextMonth}월 예상 순증감은{" "}
          <strong className="text-zinc-700 dark:text-zinc-300">
            {formatWon(projection.projectedNetChange)}
          </strong>{" "}
          입니다.
        </p>
      </div>
